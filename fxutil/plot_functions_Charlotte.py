...
def log_normal(x, sigma, mu):
    return (
        1
        / (sigma * x * math.sqrt(2 * math.pi))
        * np.exp((-1) * (np.log(x) - mu) ** 2 / (2 * sigma ** 2))
    )
...
def set_fit_function(
    ...
    if fitter == "line0":
        ...
        fitlabel = (
            r"$y={a} e^{-{b} {x}}$"  # TODO does not work due to log of negative number
        )
        ...
        fitlabel = (
            r"$y=\dfrac{(1-(2r)^2/l^2)\cdot a\cdot(l-2r)^c\cdot r^2}{(1+a\cdot(l-2r)^c\cdot r^2)}, "
            r"a={a}, c={c}$"
        )
        ...
        precision = None
        fitlabel = (
            r"$y=\dfrac{(1-(2r)^2/l^2)\cdot \alpha(l-2r)\cdot r^2}{(1+\alpha(l-2r)\cdot r^2)}$"
            + "\n"
        ...
        precision = None
        fitlabel = (
            r"$y=\dfrac{(1-(2r)^2/l^2)\cdot \alpha(r)\cdot r^2}{(1+\alpha(r)\cdot r^2)}$"
            + "\n"
        ...
        precision = None
        fitlabel = (
            r"$y=\dfrac{(1-\tilde{r}^2)\cdot \alpha(\tilde{r})\cdot \tilde{r}^2}{(1+\alpha(\tilde{r})\cdot \tilde{r}^2)}$"
            + "\n"
        ...
        precision = None
        fitlabel = (
            r"$y=\dfrac{1-2\tilde{r}+\tilde{r}^3-\tilde{r}^4}{1+{a}\cdot (1-\tilde{r})^2 x"
        ...
        function = lambda x, a, b, c: e_velocity_analytic(
            x, a, b, c, lam, v, B, lmax, ts
        )
        precision = None
        ...
        fitlabel = (
            r"$y=\dfrac{\left(\frac{64}{3}\frac{r^5}{l_{\mathrm{max}}^4}-\frac{16}{3} \frac{r^3}"
        ...
        fitlabel = (
            ...
            + "\n"
            + r"$l_{\mathrm{max}}=%5.1f,\lambda\approx%5.0f,v_\mathrm{v}=%5.0f$"
            % (lmax, lam, 1)
        )
    elif fitter == "e_load_Br":
        function = lambda x, a: e_load_B(x / 4, lmax, lam, parameter_value, 1)
        fitlabel = (
            r"$y=\dfrac{ \frac{2}{3} \ell_{\mathrm{max}}\lambda}{v_\mathrm{v}B} "
    ...
    for pool_radius_counter in range(len(unique_pool_radius)):
        ...
        for counter in range(1, width):
            ...
            try:
                ...
                means[counter - 1, pool_radius_counter] = np.mean(
                    input_array[counter, indices][notnan_indices]
                )
                errs[counter - 1, pool_radius_counter] = stats.sem(
                    input_array[counter, indices][notnan_indices], axis=None
                )
            except TypeError:
                means[counter - 1, pool_radius_counter] = np.mean(
                    input_array[counter, indices]
                )
                errs[counter - 1, pool_radius_counter] = stats.sem(
                    input_array[counter, indices], axis=None
                )
...
def add_discrete_cbar(
    ...
    bounds = np.array(
        np.concatenate(
            (
                unique_parameter,
                np.array([unique_parameter[-1] * 2 - unique_parameter[-2]]),
            )
        ),
        dtype=float,
    )
    ...
    if unique_parameter.max() <= 1:
        ...
        if len(unique_parameter) > 10:
            ...
            cbar_ticks = np.arange(
                par_min, par_max + 1, (par_max - par_min) / 4, dtype=int
            )
        else:
            cbar_ticks = unique_parameter
    ...
    if len(np.unique(dif)) > 1:
        ...
        try:
            cbar = fig.colorbar(
                scalarmappaple, ticks=cbar_ticks, cax=cbar_ax, pad=0.2, format=format
            )
        except ValueError:
            print("ValueError")
...
def plot_anything(
    ...
    xscale="linear",
    cscale="linear",
    colormap=truncate_colormap(cm.Greens, minval=0.4, maxval=1),
    color=None,
    divide_by=None,
    multiply_by=None,
    analytic=None,
    ...
    parameterset="latin",
    precision=4,
    short_axis_label=None,
    short_value_label=None,
    ylim=None,
    discrete=False,
    scientific_axis="",
    figsize=(6, 4.5),
    ...
    # print(keep_columns)
    keep_columns = np.unique(
        np.array(keep_columns)
    )  # if parameter=something else dont have column twice
    ...
    if discrete:
        ...
        evenly_spaced_interval = (unique_parameter - unique_parameter.min()) / (
            unique_parameter.max() - unique_parameter.min()
        )
        colors = [colormap(x) for x in evenly_spaced_interval]
    # print('colors',colors)
    ...
    for counter in range(len(unique_parameter)):
        ...
        if (
            len(str(parameter_value).split(".")) > 1
            and len(str(parameter_value).split(".")[-1]) >= 4
        ):
        ...
        if all_fitters:
            ...
            try:
                ...
                p0 = None
        if yerr != None and yerr != "off":
            unique_pool_radius, means, errs = calc_mean_with_error(
                np.array(
                    [
                        pool_radius[indices],
                        value_data[indices],
            ...
            unique_pool_radius, means, errs = calc_mean_with_error(
                np.array(
                    [pool_radius[indices], value_data[indices], xerr_data[indices]]
                )
            )
            value_by_pool_radius = means[0]
        ...
        if bars:
            ...
            if step:
                downsteps = (
                    np.append(
                        np.diff(unique_pool_radius)[0], np.diff(unique_pool_radius)
                    )
                    * 0.99999
                    / 2
                )
                upsteps = (
                    np.append(
                        np.diff(unique_pool_radius), np.diff(unique_pool_radius)[-1]
                    )
                    * 0.99999
                    / 2
                )
                df1 = pd.DataFrame(
                    data=np.array(
                        [unique_pool_radius - downsteps, value_by_pool_radius]
                    ).T,
                    columns=["x", "y"],
                )
                df1["sorter"] = 1
                df2 = pd.DataFrame(
                    data=np.array(
                        [unique_pool_radius + upsteps, value_by_pool_radius]
                    ).T,
                    columns=["x", "y"],
                )
                df2["sorter"] = 2
        ...
        if yerr != "off":
            axis.errorbar(
                unique_pool_radius,
                value_by_pool_radius,
                yerr=value_by_pool_radius_err,
                xerr=pool_radius_err,
                marker="",
                alpha=0.5,
                capsize=5,
                linestyle="",
        ...
        for fit_counter in range(len(fitter_vals)):
            ...
            precision = precision
            function, fitlabel, p0 = set_fit_function(
                fitter,
                lam,
                v,
                B,
            ...
            if function != None:
                try:
                    # print('p0',p0)
                    popt, pcov = curve_fit(
                        function, unique_pool_radius, value_by_pool_radius, p0=p0
                    )
                    ...
                    if True:  # R2 > 0.7:
                        ...
                        this_label = this_label.replace(
                            "+-", "-"
                        )  # if any inserted value is negative
                        this_label = this_label.replace(
                            "--", "+"
                        )  # if any inserted value is negative
                        if short_value_label != None:
        ...
        if len(R2s) > 0:
            ...
            if xlim == None:
                ...
                plot_pool_radius = np.arange(
                    unique_pool_radius.min(), unique_pool_radius.max() + span, span
                )
            else:
                # print(xlim)
            ...
            if (
                legend
                and len(unique_parameter) < 3
                or parameter == None
                and not abstract_fit
            ):
                this_label = fitlabel + "\n$R^2=%5.4f" % (R2) + "$"
                for str_counter in range(len(popt)):
                    this_label = this_label.replace(
                        "{" + ascii_lowercase[str_counter] + "}",
            ...
            this_result_df = pd.DataFrame(
                data=[this_fit_results],
                columns=["function", parameter, "R2", *popt_str],
            )
        ...
        if fitted:
            ...
            if mark_max:
                ...
                axis.annotate(
                    "",  # "{0}".format(round(value_by_pool_radius[max_index],2)),  # this is the text
                    ...
                    ha="center",
                    size=small_font,
                    arrowprops=dict(facecolor=colors[counter], shrink=0.05),
                )
            if mark_min:
                ...
                if min_index2 != min_index:
                    axis.annotate(
                        "{0}".format(
                            round(value_by_pool_radius[min_index2], 2)
                        ),  # this is the text
                        (
                            unique_pool_radius[min_index2],
                            value_by_pool_radius[min_index2],
                        ),
                        # this is the point to label
                        textcoords="offset points",  # how to position the text
                        xytext=(0, 10),  # distance from text to points (x,y)
    ...
    if type(constant) == list:
        for constant_val in constant:
            ...
            if len(constant_val) >= 3:
                axis.plot(
                    [pool_radius.min(), pool_radius.max()],
                    constant_val[0] * np.ones(2),
                    color=constant_val[1],
                    label=constant_val[2],
                    zorder=0,
                    linewidth=constlw,
                    linestyle=constls,
                )
        ...
        axis.plot(
            pool_radius,
            constant * np.ones(len(pool_radius)),
            color="black",
            linewidth=linewidth,
            linestyle="--",
        )
            ...
            if len(analytic) >= 3:
                ...
                if xscale == "log":
                    plot_pool_radius = np.exp(
                        np.linspace(
                            np.log(pool_radius.min()),
                            np.log(pool_radius.max() + span),
                            50,
                        )
                    )
                else:
            ...
            for point in points:
                if len(point) > 3:
                    if type(point[3]) == list:
                        dx = (
                            point[3][0]
                            * (unique_pool_radius.max() - unique_pool_radius.min())
                            / 100
                        )
    ...
    if title != None:
        ...
        if parameterstr != None:
            fig.suptitle(
                valuelabel + " vs " + axislabel + " by " + parameterstr,
                fontsize=large_font,
            )
        else:
    ...
    if type(vline) == list:
        ...
        axis.vlines(
            vline, ymin, ymax, color="black", linewidth=linewidth, linestyle="--"
        )
    ...
    if cbar:  # no legend but colorbar
        if cscale == "log":
            normalize = mcolors.LogNorm(
                vmin=unique_parameter[unique_parameter > 0].min(),
                vmax=unique_parameter[unique_parameter > 0].max(),
            )
        elif cscale == "linear":
    ...
    if sum_legend:
        ...
        for counter in range(len(unique_parameter)):
            ...
            labels += [
                cur_labels[
                    counter * parametervalinlegend + parametervalinlegend - 1
                ].replace("y", parameter_value)
            ]
        if len(handles) > 0:
            axis.legend(handles, labels, fontsize=small_font, loc=legendpos)
    # print(yscale)
